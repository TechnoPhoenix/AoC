...
public class Problem2 {
	...
	public static void main(String[] args) {
		...
		String input = inputToString("src/Day20_2024_12_20/input.txt");
		...
		while(xPos != xEnd || yPos != yEnd) {
			...
			switch(direction) {
				...
				case 'W':
					...
					System.out.println("No next tile found");
				default: System.out.println("Unkown direction");
			}
		}
		track.put(new Coords(xPos, yPos), Integer.valueOf(iteration));
		...
		for(Map.Entry<Coords, Integer> start : track.entrySet()) {
			for(Map.Entry<Coords, Integer> end : track.entrySet()) {
				int manhattenDistance = manhattenDistance(start.getKey(), end.getKey());
				if(manhattenDistance <= 20) {
					int cheatValue = end.getValue() - start.getValue() - manhattenDistance;
					if(cheatValue >= 100) {
						viableCheats++;
					}
				}
		...
		//output
		System.out.println("The number of cheats saving at least 100 picoseconds is: " + viableCheats);
	}
