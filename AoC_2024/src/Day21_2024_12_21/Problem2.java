...
public class Problem2 {
	...
	public static void main(String[] args) {
		...
		String input = inputToString("src/Day21_2024_12_21/input.txt");

		//split input into codes
		String[] codes = input.split("\n");
		int[] numericPart = new int[codes.length];
		for(int i = 0; i < numericPart.length; i++) {
			numericPart[i] = Integer.parseInt(codes[i].substring(0, codes[i].length() - 1));
		}
		...
		for(int i = 0; i < numericalPadOptions.length(); i++) {
			...
			for(int j = 0; j < numericalPadOptions.length(); j++) {
				boolean[][] preVisited = new boolean[numPad.length][numPad[0].length];
				numericalMap.put(new Pair(numericalPadOptions.charAt(i), numericalPadOptions.charAt(j)), new HashSet<String>());
				recursiveNumpad(numericalPadOptions.charAt(i), numericalPadOptions.charAt(j), xPos, yPos, "", preVisited);
			}
		}
		...
		for(int i = 0; i < directionalPadOptions.length(); i++) {
			...
			for(int j = 0; j < directionalPadOptions.length(); j++) {
				boolean[][] preVisited = new boolean[dirPad.length][dirPad[0].length];
				directionalMap.put(new Pair(directionalPadOptions.charAt(i), directionalPadOptions.charAt(j)), new HashSet<String>());
				recursiveDirpad(directionalPadOptions.charAt(i), directionalPadOptions.charAt(j), xPos, yPos, "", preVisited);
			}
		}
		...
		for(int i = 0; i < codes.length; i++) {
			...
			for(int j = 0; j < codes[i].length(); j++) {
				long shortest = recursiveLength(previous, codes[i].charAt(j), 0);
		...
		//output
		System.out.println("The total complexity of all codes is: " + complexitySum);
	}

	private static long recursiveLength(char previous, char next, int depth) {
		if(cache.getOrDefault(depth, new HashMap<Pair, Long>()).containsKey(new Pair(previous, next))) {
			...
			if(depth == 0) {
				for(String path : numericalMap.get(new Pair(previous, next))) {
					...
					for(int i = 0; i < path.length(); i++) {
						length += recursiveLength(prev, path.charAt(i), depth + 1);
						prev = path.charAt(i);
					}
					if(length < shortest) {
						shortest = length;
				...
				for(String path : directionalMap.get(new Pair(previous, next))) {
					long length = 0;
					char prev = 'A';
					for(int i = 0; i < path.length(); i++) {
						length += recursiveLength(prev, path.charAt(i), depth + 1);
			...
			HashMap<Pair, Long> depthCache = cache.getOrDefault(depth, new HashMap<Pair, Long>());
    ...
    private static void recursiveNumpad(char start, char end, int xPos, int yPos, String path, boolean[][] oldPreVisited) {
		...
		if(preVisited[xPos][yPos]) { return; }
	...
	private static void recursiveDirpad(char start, char end, int xPos, int yPos, String path, boolean[][] oldPreVisited) {
		if(path.length() > 8) { return; }
		try { 
