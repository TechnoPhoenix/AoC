...
public class Problem2 {
	...
	public static void main(String[] args) {
		...
		for(int i = 0; i < 75; i++) {
			...
			for(Map.Entry<Long,Long> element : counter.entrySet()) {
				...
				for(Long stone : newStones) {
					updatedCounter.put(stone, updatedCounter.getOrDefault(stone, 0l) + element.getValue());
				}
			}
		...
		System.out.println("The number of stones after blinking 75 times is: " + numberOfStones);
	}
	...
	private static ArrayList<Long> blink(Long initialStone) {
		...
		if(initialStone.longValue() == 0) {
			...
			newStones.add(Long.parseLong(initialStone.toString().substring(0, initialStone.toString().length()/2)));
			newStones.add(Long.parseLong(initialStone.toString().substring(initialStone.toString().length()/2, initialStone.toString().length())));
		} else {
