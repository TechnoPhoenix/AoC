...
public class Problem1 {
	...
	public static void main(String[] args) {
		...
		for(int i = 0; i < 25; i++) {
			...
			for(int j = 0; j < stones.size(); j++) {
				...
				if(current.longValue() == 0) {
					...
					newStones.add(Long.parseLong(current.toString().substring(0, current.toString().length()/2)));
					newStones.add(Long.parseLong(current.toString().substring(current.toString().length()/2, current.toString().length())));
				} else {
		...
		System.out.println("The number of stones after blinking 25 times is: " + stones.size());
	}
