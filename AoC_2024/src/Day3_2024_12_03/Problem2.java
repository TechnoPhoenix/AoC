...
public class Problem2 {
	public static void main(String[] args) {
		...
		String input = inputToString("src/Day3_2024_12_03/input.txt");
		...
		for(int i = 0; i < instructionBlocks.length; i++) {
			if(!instructionBlocks[i].matches("don't\\(\\).*")) {
				sumOfInstructionBlocks += performMulInstructions(instructionBlocks[i]);
			}
		}
		//output
		System.out.println("The sum of all enabled multiplications is: " + sumOfInstructionBlocks);
	}
	...
	private static int performMulInstructions(String instructionBlock) {
		...
		for(int i = 0; i < possibleMulInstructions.length; i++) {
			...
			if(!possibleMulInstructions[i].matches("mul\\([0-9]{1,3},[0-9]{1,3}\\).*")) {
				continue;
			}
			String instruction = possibleMulInstructions[i].substring(0, (possibleMulInstructions[i].indexOf(")"))) + ")"; //trim string to instruction

			//extract operants from instruction
			String operantSubstring = (instruction.substring(instruction.indexOf("(") + 1, instruction.indexOf(")")));
