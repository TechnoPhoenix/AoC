...
public class Problem1 {
	...
	public static void main(String[] args) {
		...
		HashMap<boolean[][], Integer[]> regions = new HashMap<boolean[][], Integer[]>();
		...
		for(Map.Entry<boolean[][], Integer[]> element : regions.entrySet()) {
			...
			for(int i = 0; i < element.getKey().length; i++) {
				for(int j = 0; j < element.getKey()[0].length; j++) {
					try {
						if(element.getKey()[i][j] != element.getKey()[i + 1][j]) { perimeter++; }
					} catch(ArrayIndexOutOfBoundsException e) {
						//nothing
					}
					try {
						if(element.getKey()[i][j] != element.getKey()[i][j + 1]) { perimeter++; }
					} catch(ArrayIndexOutOfBoundsException e) {
						//nothing
					}
				}
			...
			for(int i = 0; i < element.getKey().length; i++) {
				if(element.getKey()[0][i]) { perimeter++; }
				if(element.getKey()[element.getKey().length - 1][i]) { perimeter++; }
				if(element.getKey()[i][0]) { perimeter++; }
				if(element.getKey()[i][element.getKey().length - 1]) { perimeter++; }
			}

			Integer[] value = {Integer.valueOf(area), Integer.valueOf(perimeter)};
		...
		System.out.println("The total cost of fencing all regions is: " + totalCost);
	}
	...
	private static void recIdentify(char[][] map, boolean[][] currentRegion, int x, int y, char origin) {
		try {
