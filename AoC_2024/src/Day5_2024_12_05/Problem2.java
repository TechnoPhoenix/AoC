...
public class Problem2 {
	...
	public static void main(String[] args) {
		...
		String input = inputToString("src/Day5_2024_12_05/input.txt");
				...
				for(int k = 0; k < orderingRules.length; k++) {
					if(update[j] == orderingRules[k][1]) {
						if(isInArray(update, orderingRules[k][0])) {
							if(!isRuleFullfilled(update, orderingRules[k][0], j)) {
								incorrect[i] = true;
							}
						}
					}
		...
		for(int i = 0; i < incorrect.length; i++) {
			if(incorrect[i]) {
				sumOfCorrectedUpdates += reorderCorrectly(updates[i], orderingRules);
			}
		}

		//output
		System.out.println("The sum of all middle elements of correct updates is: " + sumOfCorrectedUpdates);
	}
	...
	private static boolean isRuleFullfilled(int[] updates, int requiredInt, int indexOfElement) {
