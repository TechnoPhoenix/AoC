...
public class Problem1 {
	...
	public static void main(String[] args) {
		...
		String input = inputToString("src/Day8_2024_12_08/input.txt");
		...
		for(int i = 0; i < (map.length*map[0].length); i++) {
			if(map[i/map.length][i%map[0].length] != '.') {
				for(int j = i + 1; j < (map.length*map[0].length); j++) {
					if(map[i/map.length][i%map[0].length] == map[j/map.length][j%map[0].length]) {
						...
						if(x1 < locations[0].length && x1 >= 0 && y1 < locations.length && y1 >= 0) {
						...
						if(x2 < locations[0].length && x2 >= 0 && y2 < locations.length && y2 >= 0) {
		...
		System.out.println("Total number of locations with antinodes is: " + locationCount);
	}
