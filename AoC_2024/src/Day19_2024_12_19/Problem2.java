...
public class Problem2 {
	...
	public static void main(String[] args) {
		...
		String input = inputToString("src/Day19_2024_12_19/input.txt");
		...
		for(int i = 0; i < availableTowels.length; i++) {
			...
			possibleTowels.get(availableTowels[i].charAt(0)).add(availableTowels[i]);
		}
		...
		for(int i = 0; i < desiredPatterns.length; i++) {
			possibleArrangements += calcPossibleArrangements(desiredPatterns[i]);
		}

		//output
		System.out.println("The number of possible arrangements is: " + possibleArrangements);
	}

	private static long calcPossibleArrangements(String desiredPattern) {
		if(desiredPattern.isEmpty()) { return 1; }
		if(cache.containsKey(desiredPattern)) { return cache.get(desiredPattern); }

		long possibleArrangements = 0;
		ArrayList<String> list = possibleTowels.get(desiredPattern.charAt(0));
		for(String towel : list) {
			if(towel.length() > desiredPattern.length()) { continue; }
			if(desiredPattern.startsWith(towel)) {
				String remainingPattern = desiredPattern.substring(towel.length());
				possibleArrangements += calcPossibleArrangements(remainingPattern);
			}
		}
