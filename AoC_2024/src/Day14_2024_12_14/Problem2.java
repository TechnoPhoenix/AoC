	...
	public static void main(String[] args) {
		...
		while(passedSeconds < 100000) {
			...
			for(String robot : robots) {
				...
				int xPosEnd = (((xPos + (passedSeconds * xVel)) % robotPositions.length) + robotPositions.length) % robotPositions.length;
				int yPosEnd = (((yPos + (passedSeconds * yVel)) % robotPositions[0].length) + robotPositions[0].length) % robotPositions[0].length;
			...
			if(found) {
				...
				System.out.println("Tree found after " + passedSeconds + " seconds");
				break;
			}
